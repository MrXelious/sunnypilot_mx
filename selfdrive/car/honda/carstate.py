...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.prev_cruiseState_available = False
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    self.prev_cruiseState_available = ret.cruiseState.available
    ...
    self.persistMainDisabled = not ret.cruiseState.available
    ...
    if ret.cruiseState.available:
      ...
      if self.enable_mads:
        if (self.prev_cruise_setting != 1 and self.cruise_setting == 1) or \
           (not self.prev_cruiseState_available and ret.cruiseState.available):
